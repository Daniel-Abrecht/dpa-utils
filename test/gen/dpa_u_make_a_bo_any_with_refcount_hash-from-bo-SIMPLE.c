...
int main(){
  dpa_u_refcount_freeable_data_t* refdata = dpa_u_refcount_freeable_copy("3", 1, 1);
  const dpa_u_bo_t bo = {
    .size=1,
    .data=refdata->data
  };
const dpa_u_bo_t inbo = bo;
  ...
  const dpa_u_a_bo_any_t outbo = dpa_u_make_a_bo_any_with_refcount_hash(inbo, refcount, hash);
