...
dpa__u_api inline dpa__u_boptr_t dpa__u_bo__alloc_p_any_bo_do_hash(dpa_u_bo_t bo, dpa__u_bo_hashed_t*restrict cmem){
  *cmem = (dpa__u_bo_hashed_t){
    .bo = bo,
    .hash = dpa__u_bo_hash(bo),
  };
  return DPA__U_BO_TAG(&cmem->bo, DPA_U_BO_SIMPLE|DPA_U_BO_HASHED);
}
