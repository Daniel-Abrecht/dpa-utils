...
#define dpa__u_v_bo_refcounted_ro__helper(P,X) 0 // (const dpa_u_bo_refcount_ro_t){ .bo_simple = { .type = DPA_U_BO_REFCOUNTED, .size=dpa__u_ ## P ## __get_size((X)), .data=dpa__u_ ## P ## __data((X)) }, .refcount=dpa__u_ ## P ## __get_refcount((X)) }
